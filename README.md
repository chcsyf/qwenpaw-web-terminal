...
> v0.2.5 起修复 **PTY I/O 阻塞 asyncio 事件循环**（破坏性：曾导致主服务约 40s 无响应）；
> v0.2.4 起内置 SSE 降级（网关不透传 WebSocket `Upgrade` 头时自动切换），并修复开启登录认证部署下的 401 与终端空白问题；本地与平台部署**统一使用同一版本**，不再需要单独的分支版本。
