# 🖥️ Web 终端 (qwenpaw-web-terminal) v0.1.2
...
- **会话持久化**：WS 意外断开（刷新页面/断网）**不再 kill PTY 进程**——终端转入后台保留，
  输出写入 64KB 环形缓冲；重新连接同会话自动 attach 原进程并回放断开期间的输出。
  正常关闭标签（×）仍是「关闭即结束」，不会产生僵尸进程
- **会话管理面板**：标签栏「🗂 会话管理」按钮弹出管理面板，列出全部会话：
  状态徽标（● 运行中·已连接 / ◉ 后台运行·缓冲 N KB / 空闲）、PID、最后活动、cwd，
  支持**打开**（attach 后台进程）、**结束**（结束进程、保留会话）、**删除**（会话+进程）、
  **结束所有后台会话**（一键清理不在前台标签栏的运行中会话）、**新建会话**；
  会话 id 前 📌 标记「前台已打开」的会话
- **同名新建自动打开**：新建终端时若名称与已有会话一致，直接打开现有会话
  （不重置 cwd、不重复创建）；初始化只恢复上次打开的标签，不自动创建
  default/智能体同名会话（惰性连接，不持续运行）
- **心跳保活**：前端每 20s 发一次心跳，后端自动清理超过 60s 无消息的
  死连接（标签冻结/断网/页面强制销毁残留的「已连接」幽灵会话）
- **default 会话不持久化**：`default` 是兜底会话，断开（刷新/断网）即结束进程、
  不进入后台；其他命名会话支持后台保留 + 缓冲回放
- **全量历史回放**：始终记录会话输出（上限 256KB），刷新/重连后终端恢复
  断点前内容（命令回显、输出、prompt），断开期间的新输出也一并回放
- **标签布局持久化**：打开的标签列表存 localStorage，刷新后恢复；后端会话全量收敛到
  管理面板，标签栏不会无限堆积
...
> 原进程并回放后台缓冲；WS 断开（刷新/断网）进程保留后台运行，输出写入 64KB 缓冲。
